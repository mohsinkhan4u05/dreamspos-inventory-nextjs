...
interface SalesOrderSendEmailFormProps {
  id: string;
}

export default function SalesOrderSendEmailPage(props: any) {
  const { id } = (props.params ?? {}) as { id: string };
  return <SalesOrderSendEmailForm id={id} />;
}

function SalesOrderSendEmailForm({ id }: SalesOrderSendEmailFormProps) {
  const router = useRouter();
  const route = all_routes;
  const { order, loading, error } = useSalesOrder(id);
  ...
  useEffect(() => {
    ...
    if (!subject) {
      setSubject(`Sales Order #${order.orderNumber}`);
    }
  }, [order, to, subject]);
    ...
    return (
      <div className="page-wrapper">
        <div className="content">
          <p>Sales order id is required.</p>
          <Link
            href={route.salesorders || "/sales-orders"}
  ...
  const sendEmailInternal = async (includePdf: boolean) => {
    ...
    try {
      ...
      setSendError(
        err instanceof Error ? err.message : "Failed to send sales order email",
      );
    } finally {
      setSending(false);
    }
