...
export default function BillDetail({ id, purchaseOrderId, autoOpenPayment }: Props) {
  ...
  const [hasAutoOpenedPayment, setHasAutoOpenedPayment] = useState(false);

  useEffect(() => {
    if (
      autoOpenPayment &&
      !hasAutoOpenedPayment &&
      bill &&
      typeof bill.dueAmount === "number" &&
      bill.dueAmount > 0
    ) {
      setHasAutoOpenedPayment(true);
      setPaymentError(null);
      setPaymentAmount(bill.dueAmount.toFixed(2));
      setPaymentMethod("CASH");
      setIsPaymentModalOpen(true);
    }
  }, [autoOpenPayment, hasAutoOpenedPayment, bill]);

  if (!id) {
    return (
      <div className="page-wrapper">
        <div className="content">
          <p>Bill id is required.</p>
          <Link
  ...
  if (loading || !bill) {
    return (
      <div className="page-wrapper">
        <div className="content">
          <p>{loading ? "Loading bill..." : error || "Bill not found"}</p>
        </div>
      </div>
    );
  }
  ...
  const handleSavePayment = async () => {
    ...
    try {
      ...
      setPaymentError(null);
      await purchasePaymentService.recordPayment(bill.id, {
        amount: parsed,
        paymentMethod,
      });
      await refetch();
        ...
        <div className="content">
          <div className="page-header">
            <div className="page-title">
              <h4>Bill #{bill.orderNumber}</h4>
              <h6>{bill.supplier?.name || ""}</h6>
              {purchaseOrderId && (
                <div className="mt-1">
                  <Link
            ...
            <div className="page-btn d-flex align-items-center gap-2">
              {renderPaymentStatusBadge(bill.paymentStatus)}
              {bill.dueAmount > 0 && (
                <button
                  type="button"
            ...
            <div className="card-body d-flex justify-content-between flex-wrap gap-3">
              <div>
                ...
                <p className="mb-1">
                  ...
                  {formatDate(bill.createdAt || bill.expectedDate || "")}
                </p>
                <p className="mb-1">
                  <strong>Store:</strong> {bill.store?.name || "-"}
                </p>
              </div>
              <div className="text-end">
                ...
                <p className="mb-1">
                  <strong>Due:</strong> {formatCurrency(bill.dueAmount)}
                </p>
              </div>
            </div>
          </div>
